...
export class UploadingComponent implements OnInit {
  ...
  applyCrop(): void {
    if (!this.editCanvas) return; // Ensure the canvas exists
    ...
    if (width > 0 && height > 0) {
      // Get the image data for the specified cropping area
      ...
      // Put the cropped image data back onto the canvas
      ctx.putImageData(imageData, 0, 0);

      // Update the edited image URL with the new cropped image
      this.editImageUrl = canvas.toDataURL('image/png');
  ...
  resetCropVariables(): void {
    this.cropStartX = 0;
    this.cropStartY = 0;
    this.cropEndX = 0;
    this.cropEndY = 0;
    this.cropping = false; // Optional, based on your flow
  }
